...
export interface SolarResult {
  ...
  civil_time_ambiguous?:boolean;
  civil_time_candidates?:string[];
}

export interface ZonedCivilResolution {
  utc_epoch_ms:number;
  utc_offset_minutes:number;
  timezone_id:string;
  ambiguous:boolean;
...
export class TrueSolarTimeCalculator {
  ...
  private static isLeapYear(year:number):boolean{return year%400===0||(year%4===0&&year%100!==0);}
  private static daysInMonth(year:number,month:number):number{return [31,this.isLeapYear(year)?29:28,31,30,31,30,31,31,30,31,30,31][month-1]??0;}
  static isValidCivilDate(year:number,month:number,day:number):boolean {
  ...
  private static formatInZone(epochMs:number,timeZone:string):number[]{
    const fmt=new Intl.DateTimeFormat('en-CA',{timeZone,year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',hourCycle:'h23'}),parts:any={};
    for(const p of fmt.formatToParts(new Date(epochMs)))if(p.type!=='literal')parts[p.type]=Number(p.value);
    return [parts.year,parts.month,parts.day,parts.hour,parts.minute,parts.second];
  }
  private static sameCivil(a:number[],b:number[]):boolean{return a.slice(0,5).every((v,i)=>v===b[i]);}
  private static offsetAtInstant(epochMs:number,timeZone:string):number{
  ...
  static resolveZonedCivilTime(birthday:string,birthTime:string,timeZone:string):ZonedCivilResolution {
    if(!this.isValidTimeZone(timeZone))throw new Error(`无效 IANA 时区：${timeZone}`);
    const dm=/^(\d{4})-(\d{2})-(\d{2})$/.exec(birthday),tm=/^(\d{2}):(\d{2})$/.exec(birthTime);
    if(!dm||!tm)throw new Error('出生日期或时间格式无效');
    const local=[+dm[1],+dm[2],+dm[3],+tm[1],+tm[2],0];
    const pseudo=Date.UTC(local[0],local[1]-1,local[2],local[3],local[4],0);
    let guess=pseudo;
    for(let i=0;i<4;i++){const off=this.offsetAtInstant(guess,timeZone);guess=pseudo-off*60000;}
    const candidates:number[]=[];
    for(let delta=-180;delta<=180;delta+=15){const c=guess+delta*60000;if(this.sameCivil(this.formatInZone(c,timeZone),local)&&!candidates.includes(c))candidates.push(c);}
    if(!candidates.length)throw new Error(`出生时间 ${birthday} ${birthTime} 在 ${timeZone} 不存在，可能处于夏令时跳时区间，请核对原始出生记录`);
  ...
  static calculate(birthday:string,birthTime:string,longitude:number,timezone:number|string=8):SolarResult {
    const dm=/^(\d{4})-(\d{2})-(\d{2})$/.exec(birthday),tm=/^(\d{2}):(\d{2})$/.exec(birthTime);
    if(!dm||!tm)throw new Error('出生日期或时间格式无效');
    const y=+dm[1],m=+dm[2],d=+dm[3],hh=+tm[1],mi=+tm[2];
    if(!this.isValidCivilDate(y,m,d))throw new Error('出生日期无效');
    if(hh<0||hh>23||mi<0||mi>59)throw new Error('出生时间无效');
    if(!Number.isFinite(longitude)||longitude<-180||longitude>180)throw new Error('出生地经度无效');

    let utcEpochMs:number,offsetMinutes:number,timeZoneId='',ambiguous=false,candidates:number[]=[];
    if(typeof timezone==='string'){
      const z=this.resolveZonedCivilTime(birthday,birthTime,timezone);utcEpochMs=z.utc_epoch_ms;offsetMinutes=z.utc_offset_minutes;timeZoneId=z.timezone_id;ambiguous=z.ambiguous;candidates=z.candidates;
    }else{
      if(!Number.isFinite(timezone)||timezone<-14||timezone>14)throw new Error('出生地时区偏移无效');
      offsetMinutes=Math.round(timezone*60);utcEpochMs=Date.UTC(y,m-1,d,hh,mi,0)-offsetMinutes*60000;
    }
