...
import {TrueSolarTimeCalculator} from './core/TrueSolarTimeCalculator';
import {LocalPaipan} from './core/LocalPaipan';
...
function parseGender(value:any):'male'|'female'{
  if(value===undefined||value===null||value==='')return 'male';
  if(value==='male'||value===0||value==='0')return 'male';
  if(value==='female'||value===1||value==='1')return 'female';
  const s=String(value).trim().toLowerCase();
  if(['male','man','m','男'].includes(s))return 'male';
  if(['female','woman','f','女'].includes(s))return 'female';
  throw new Error('性别参数无效');
}

function attachShenSha(adapter:LocalChartAdapter,chart:any,gender:'male'|'female'){
  ...
  for(const rule of chart.shen_sha_full)for(const match of rule.matches??[]){
    const key=pillarMap[match.target_pillar??''],name=String(rule.name??'').trim();
    if(key&&name&&!chart.shen_sha[key].includes(name))chart.shen_sha[key].push(name);
  }
...
function attachBlindJudgment(chart:any){
  try{
...
export function computePaipan(input:any):ApiResponse {
  ...
  try{ gender=parseGender(input.gender); }
  ...
  if(!dm||!tm)return {ok:false,message:'请填写完整的出生日期和时间'};
  const y=Number(dm[1]),m=Number(dm[2]),d=Number(dm[3]),hh=Number(tm[1]),mi=Number(tm[2]);
  if(hh<0||hh>23||mi<0||mi>59)return {ok:false,message:'出生时间无效'};
  ...
  if(!isLunar&&!TrueSolarTimeCalculator.isValidCivilDate(y,m,d))return {ok:false,message:'出生日期无效'};
  if(isLunar){
    if(m<1||m>12||d<1||d>30)return {ok:false,message:'农历出生日期无效'};
    const lunarEngine=new LocalPaipan(),leapMonth=lunarEngine.GetLeap(y),days=lunarEngine.GetLunarDays(y,m,isLeap?1:0);
    if((isLeap&&leapMonth!==m)||days<=0||d>days)return {ok:false,message:'农历出生日期无效'};
  }
  if(useTrueSolarTime&&(longitude===null||!Number.isFinite(longitude)||longitude<-180||longitude>180))return {ok:false,message:'启用真太阳时时，请填写 -180—180 之间的有效出生地经度'};
  if(useTrueSolarTime&&longitude===0&&!timezoneId&&(timezoneOffsetRaw===null||timezoneOffsetRaw===undefined||timezoneOffsetRaw===''))return {ok:false,message:'启用真太阳时时，请填写有效的出生地经度'};
  if(!useTrueSolarTime&&longitude!==null&&(!Number.isFinite(longitude)||longitude<-180||longitude>180))return {ok:false,message:'出生地经度无效'};
  if(timezoneId&&!TrueSolarTimeCalculator.isValidTimeZone(timezoneId))return {ok:false,message:'出生地 IANA 时区无效，例如 Asia/Shanghai、America/New_York'};
  const timezoneOffset=(timezoneOffsetRaw===null||timezoneOffsetRaw===undefined||timezoneOffsetRaw==='')?8:Number(timezoneOffsetRaw);
  if(!timezoneId&&(!Number.isFinite(timezoneOffset)||timezoneOffset<-14||timezoneOffset>14))return {ok:false,message:'出生地 UTC 时区偏移应在 -14 到 +14 之间'};
  try{
    ...
    const adapter=new LocalChartAdapter(),chart=adapter.compute(profile);
    attachShenSha(adapter,chart,gender);
    attachBlindJudgment(chart);
    return {ok:true,data:chart};
  }catch(e:any){return {ok:false,message:e?.message??String(e)};}
