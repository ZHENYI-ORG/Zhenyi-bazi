...
function computePaipan(input) {
    ...
    if (!(longitude > 0 && longitude <= 180))
        return { ok: false, message: '请填写有效的出生地经度' };
    try {
        ...
        const adapter = new LocalChartAdapter_1.LocalChartAdapter(), chart = adapter.compute(profile), shenSha = adapter.computeShenSha(chart, profile.gender === 'male' ? 0 : 1);
