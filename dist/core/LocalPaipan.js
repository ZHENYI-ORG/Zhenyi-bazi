...
class LocalPaipan {
    ...
    dzwx = [4, 2, 0, 0, 2, 1, 1, 2, 3, 3, 2, 4];
    dztg = [8, 5, 0, 2, 4, 3, 2, 5, 6, 7, 4, 9];
    dzcg = [[9], [5, 7, 9], [0, 2, 4], [1], [4, 9, 1], [2, 6, 4], [3, 5], [5, 1, 3], [6, 8, 4], [7], [4, 3, 7], [8, 0]];
    csa = ['鼠', '牛', '虎', '兔', '龙', '蛇', '马', '羊', '猴', '鸡', '狗', '猪'];
    cxz = ['水瓶座', '双鱼座', '白羊座', '金牛座', '双子座', '巨蟹座', '狮子座', '处女座', '天秤座', '天蝎座', '射手座', '摩羯座'];
    wkd = ['日', '一', '二', '三', '四', '五', '六'];
    jq = ['春分', '清明', '谷雨', '立夏', '小满', '芒种', '夏至', '小暑', '大暑', '立秋', '处暑', '白露', '秋分', '寒露', '霜降', '立冬', '小雪', '大雪', '冬至', '小寒', '大寒', '立春', '雨水', '惊蛰'];
    synmonth = 29.530588853;
    ten_god = [['比', '劫'], ['食', '伤'], ['财', '才'], ['杀', '官'], ['枭', '印']];
    cs = ['生', '沐', '冠', '临', '旺', '衰', '病', '死', '墓', '绝', '胎', '养'];
    selfQi = ['禄', '刃', '长生', '墓库', '余气', '死', '绝'];
    cs_tg2dz = [11, 6, 2, 9, 2, 9, 5, 0, 8, 3];
    ...
    DeltaT(yy, mm) {
        ...
        if (y <= -500) {
            ...
            u = y / 100;
            dt = 10583.6 - 1014.41 * u + 33.78311 * u * u - 5.952053 * u ** 3 - 0.1798452 * u ** 4 + 0.022174192 * u ** 5 + 0.0090316521 * u ** 6;
        }
        else if (y < 1600) {
            u = (y - 1000) / 100;
            dt = 1574.2 - 556.01 * u + 71.23472 * u * u + 0.319781 * u ** 3 - 0.8503463 * u ** 4 - 0.005050998 * u ** 5 + 0.0083572073 * u ** 6;
        }
        else if (y < 1700) {
            t = y - 1600;
            ...
            t = y - 1860;
            dt = 7.62 + 0.5737 * t - 0.251754 * t * t + 0.01680668 * t ** 3 - 0.0004473624 * t ** 4 + t ** 5 / 233174;
        }
        else if (y < 1920) {
            t = y - 1900;
            ...
            t = y - 1920;
            dt = 21.2 + 0.84493 * t - 0.0761 * t * t + 0.0020936 * t ** 3;
        }
        else if (y < 1961) {
            t = y - 1950;
            ...
            t = y - 2000;
            dt = 63.86 + 0.3345 * t - 0.060374 * t * t + 0.0017275 * t ** 3 + 0.000651814 * t ** 4 + 0.00002373599 * t ** 5;
        }
        else if (y < 2050) {
            t = y - 2000;
    ...
    MeanJQJD(yy) {
        ...
        const rvp = vp * 2 * Math.PI / 360, peri = [];
        for (let i = 0; i < num; i++) {
    ...
    GetAdjustedJQ(yy, start, end) {
        if (start < 0 || start > 25 || end < 0 || end > 25)
            return [];
        const out = [];
        const jqjd = this.MeanJQJD(yy);
        jqjd.forEach((jd, k) => { if (k < start || k > end)
            return; const ptb = this.Perturbation(jd); const dt = this.DeltaT(yy, Math.floor((k + 1) / 2) + 3); out[k] = jd + ptb - dt / 60 / 24 + 1 / 3; });
        return out;
    }
    GetPureJQsinceSpring(yy) {
        const out = [];
        let dj = this.GetAdjustedJQ(yy - 1, 19, 23);
        Object.keys(dj).forEach(ks => { const k = +ks; if (k >= 19 && k <= 23 && k % 2 !== 0)
            out.push(dj[k]); });
        dj = this.GetAdjustedJQ(yy, 0, 25);
        Object.keys(dj).forEach(ks => { const k = +ks; if (k % 2 !== 0)
            out.push(dj[k]); });
        return out;
    }
    GetZQsinceWinterSolstice(yy) {
        const out = [];
        let dj = this.GetAdjustedJQ(yy - 1, 18, 23);
        out[0] = dj[18];
        out[1] = dj[20];
        out[2] = dj[22];
        dj = this.GetAdjustedJQ(yy, 0, 23);
        Object.keys(dj).forEach(ks => { const k = +ks; if (k % 2 === 0)
            out.push(dj[k]); });
        return out;
    }
    ...
    MeanNewMoon(jd) { const kn = Math.floor((jd - 2451550.09765) / this.synmonth), jdt = 2451550.09765 + kn * this.synmonth, t = (jdt - 2451545) / 36525; return [kn, jdt + 0.0001337 * t * t - 0.00000015 * t ** 3 + 0.00000000073 * t ** 4]; }
    Julian2Solar(jd) {
        let y4h, init;
        if (jd >= 2299160.5) {
            y4h = 146097;
        ...
        const jdr = Math.floor(jd - init), yh = y4h / 4, cen = Math.floor((jdr + 0.75) / yh);
        let d = Math.floor(jdr + 0.75 - cen * yh);
        const ywl = 1461 / 4, jy = Math.floor((d + 0.75) / ywl);
        d = Math.floor(d + 0.75 - ywl * jy + 1);
        const ml = 153 / 5, mp = Math.floor((d - 0.5) / ml);
    ...
    GetSMsinceWinterSolstice(yy, jdws) {
        const tjd = [];
        const jd = this.Solar2Julian(yy - 1, 11, 1, 0, 0, 0);
        if (jd === false)
            return [];
        const [kn, thejd] = this.MeanNewMoon(jd);
        for (let i = 0; i <= 19; i++) {
            const k = kn + i;
            tjd[i] = this.TrueNewMoon(k) + 1 / 3 - this.DeltaT(yy, i - 1) / 1440;
        }
        let j = 0;
        for (j = 0; j <= 18; j++) {
    ...
    Solar2Julian(yy, mm, dd, hh = 0, mt = 0, ss = 0) {
        if (!this.ValidDate(yy, mm, dd) || hh < 0 || hh >= 24 || mt < 0 || mt >= 60 || ss < 0 || ss >= 60)
            return false;
        ...
        if (init === undefined || jdy === undefined)
            return false;
        const mp = Math.floor(mm + 9) % 12, jdm = mp * 30 + Math.floor((mp + 1) * 34 / 57), jdd = dd - 1, jdh = (hh + (mt + ss / 60) / 60) / 24;
        return jdy + jdm + jdd + jdh + init;
    }
    ValidDate(yy, mm, dd) {
        if (yy < -1000 || yy > 3000 || mm < 1 || mm > 12 || (yy === 1582 && mm === 10 && dd >= 5 && dd < 15))
            return false;
        const ndf1 = -(yy % 4 === 0 ? 1 : 0), ndf2 = ((yy % 400 === 0 ? 1 : 0) - (yy % 100 === 0 ? 1 : 0)) && yy > 1582 ? 1 : 0, ndf = ndf1 + ndf2;
        const dom = 30 + ((Math.abs(mm - 7.5) + 0.5) % 2) - (mm === 2 ? 1 : 0) * (2 + ndf);
        return dd > 0 && dd <= dom;
    }
    GetSolarDays(yy, mm) { if (yy < -1000 || yy > 3000 || mm < 1 || mm > 12)
        return 0; const ndf1 = -(yy % 4 === 0 ? 1 : 0), ndf2 = ((yy % 400 === 0 ? 1 : 0) - (yy % 100 === 0 ? 1 : 0)) && yy > 1582 ? 1 : 0, ndf = ndf1 + ndf2; return 30 + ((Math.abs(mm - 7.5) + 0.5) % 2) - (mm === 2 ? 1 : 0) * (2 + ndf); }
    GetLunarDays(yy, mm, isLeap) {
        if (yy < -1000 || yy > 3000 || mm < 1 || mm > 12)
            return 0;
        const [, jdnm, mc] = this.GetZQandSMandLunarMonthCode(yy);
        let leap = 0;
        for (let j = 1; j <= 14; j++) {
        ...
        const nofd = [];
        for (let i = 0; i <= 14; i++)
            nofd[i] = Math.floor(jdnm[i + 1] + 0.5) - Math.floor(jdnm[i] + 0.5);
        let dy = 0;
        if (isLeap) {
            if (leap >= 3 && leap === mm)
                dy = nofd[mm];
        ...
        return Math.trunc(dy);
    }
    GetLeap(yy) { const [, , mc] = this.GetZQandSMandLunarMonthCode(yy); let leap = 0; for (let j = 1; j <= 14; j++) {
        if (mc[j] - Math.floor(mc[j]) > 0) {
            leap = Math.floor(mc[j] + 0.5);
            break;
        }
    } return Math.max(0, leap - 2); }
    GetZodiac(mm, dd) { if (mm < 1 || mm > 12 || dd < 1 || dd > 31)
        return false; const dds = [20, 19, 21, 20, 21, 22, 23, 23, 23, 24, 22, 22]; let kn = mm - 1; if (dd < dds[kn])
        kn = this.mod(kn - 1, 12); return kn; }
    GetWeek(yy, mm, dd) { const jd = this.Solar2Julian(yy, mm, dd, 12); if (jd === false)
        return false; return this.mod(Math.floor(jd + 1), 7); }
    Lunar2Solar(yy, mm, dd, isLeap) {
        if (yy < -1000 || yy > 3000 || mm < 1 || mm > 12 || dd < 1 || dd > 30)
            return false;
        const [, jdnm, mc] = this.GetZQandSMandLunarMonthCode(yy);
        let leap = 0;
        for (let j = 1; j <= 14; j++) {
    ...
    Solar2Lunar(yy, mm, dd) {
        if (!this.ValidDate(yy, mm, dd))
            return false;
        let prev = 0, isLeap = 0;
        let [jdzq, jdnm, mc] = this.GetZQandSMandLunarMonthCode(yy);
        const jd = this.Solar2Julian(yy, mm, dd, 12, 0, 0);
        if (jd === false)
            return false;
        if (Math.floor(jd) < Math.floor(jdnm[0] + 0.5)) {
            prev = 1;
            [jdzq, jdnm, mc] = this.GetZQandSMandLunarMonthCode(yy - 1);
        }
        let mi = 0;
        for (let i = 0; i <= 14; i++) {
        ...
        if ((mc[mi] - Math.floor(mc[mi])) * 2 + 1 !== 1)
            isLeap = 1;
        mm = (Math.floor(mc[mi] + 10) % 12) + 1;
        dd = Math.floor(jd) - Math.floor(jdnm[mi] + 0.5) + 1;
        return [yy, mm, dd, isLeap];
    }
    Get24JieQi(yy) { const out = []; let dj = this.GetAdjustedJQ(yy - 1, 21, 23); Object.keys(dj).forEach(ks => { const k = +ks; if (k >= 21 && k <= 23)
        out.push(this.Julian2Solar(dj[k])); }); dj = this.GetAdjustedJQ(yy, 0, 20); Object.keys(dj).forEach(ks => out.push(this.Julian2Solar(dj[+ks]))); return out; }
    GetGanZhi(yy, mm, dd, hh, mt = 0, ss = 0) {
        const jd = this.Solar2Julian(yy, mm, dd, hh, mt, Math.max(1, ss));
        if (jd === false)
            return [];
        const tg = [], dz = [];
        let jq = this.GetPureJQsinceSpring(yy);
        if (jd < jq[1]) {
            yy--;
            jq = this.GetPureJQsinceSpring(yy);
        }
        const ygz = this.mod(yy + 4712 + 24, 60);
        tg[0] = ygz % 10;
        dz[0] = ygz % 12;
        let ix = 0;
        for (let j = 0; j <= 15; j++) {
        ...
        const tmm = this.mod((yy + 4712) * 12 + (ix - 1) + 60, 60), mgz = (tmm + 50) % 60;
        tg[1] = mgz % 10;
        dz[1] = mgz % 12;
        const jda = jd + 0.5, thes = (jda - Math.floor(jda)) * 86400 + 3600, dayjd = Math.floor(jda) + thes / 86400, dgz = this.mod(Math.floor(dayjd + 49), 60);
        tg[2] = dgz % 10;
        dz[2] = dgz % 12;
        if (this.zwz && hh >= 23) {
            tg[2] = this.mod(tg[2] - 1, 10);
            dz[2] = this.mod(dz[2] - 1, 12);
        }
        const hgz = this.mod(Math.floor(dayjd * 12 + 48), 60);
        tg[3] = hgz % 10;
        dz[3] = hgz % 12;
        return [tg, dz, jd, jq, ix];
    }
    GetXiongWang(dayTg, dayDz) { let s = dayDz - dayTg - 2; if (s < 0)
        s += 12; if (s === 12)
        s = 0; let e = s + 1; if (e === 12)
        e = 0; return { index: [s, e], char: this.cdz[s] + this.cdz[e] }; }
    GetTenGod(dayTg, otherTg) { const l2 = (dayTg + otherTg) % 2, dayWx = this.GetTgWx(dayTg), otherWx = this.GetTgWx(otherTg); let l1 = otherWx - dayWx; if (l1 < 0)
        l1 += 5; return { index: [l1, l2], char: this.ten_god[l1][l2] }; }
    GetTgWx(tg) { if (tg % 2)
        tg--; return tg / 2; }
    getSanHe(dz) { const fir = (dz % 4) * 3, sec = (fir + 4) % 12, thr = (sec + 4) % 12, ju = [4, 0, 1, 3][fir / 3]; return { sanhe: [fir, sec, thr], ju }; }
    getChong(dz) { return (dz + 6) % 12; }
    getXingFrom(dz) { return [3, 7, 5, 0, 4, 8, 6, 10, 2, 9, 1, 11][dz]; }
    tgHe(monthDz, tgA) { if (tgA > 11)
        return false; const from = tgA >= 5 ? tgA - 5 : tgA, tag = tgA >= 5 ? tgA : tgA + 5; const map = [[1, 4, 5, 6, 7, 10], [4, 8, 9, 10, 1], [8, 9, 11, 0, 1], [2, 3, 4, 11, 0, 1], [2, 3, 4, 5, 6, 7]], ju = [2, 3, 4, 0, 1]; return map[from].includes(monthDz) ? { he: [from, tag], ju: ju[from] } : false; }
    getLiuHe(dz) { const tmp = dz === 0 ? 12 : dz, he = (13 - tmp) % 12, hua = [2, 2, 0, 1, 3, 4, 2], ju = dz < he ? hua[dz] : hua[he]; return { index: he, ju }; }
    getChuan(dz) { return (19 - dz) % 12; }
    getPo(dz) { return [9, 4, 11, 6, 1, 8, 6, 10, 5, 0, 2, 11][dz]; }
    GetCs(tg, dz) { const base = this.cs_tg2dz[tg]; let move = tg % 2 === 0 ? dz - base : base - dz; if (move < 0)
        move += 12; return { index: move, char: this.cs[move] }; }
    GetGong(yearTg, monthDz, hourDz) { const gd = (29 - monthDz - hourDz) % 12, xi = gd < 2 ? 1 : 0, gt = ((yearTg % 5) * 2 + gd + 12 * xi) % 10; return { index: [gt, gd], char: this.ctg[gt] + this.cdz[gd] }; }
    GetShenGong(yearTg, monthDz, hourDz) { const dz = (monthDz + hourDz + 1) % 12, xi = dz < 2 ? 1 : 0, tg = ((yearTg % 5) * 2 + dz + 12 * xi) % 10; return { index: [tg, dz], char: this.ctg[tg] + this.cdz[dz] }; }
    GetTaiXi(dayTg, dayDz) { const tg = [5, 6, 7, 8, 9, 0, 1, 2, 3, 4][dayTg], dz = [1, 0, 11, 10, 9, 8, 7, 6, 5, 4, 3, 2][dayDz]; return { index: [tg, dz], char: this.ctg[tg] + this.cdz[dz] }; }
    GetCTPart(hh, ii) { const des = ['时头', '时中', '时尾']; hh = Math.trunc(hh); ii = Math.trunc(ii); const sc = Math.trunc((hh + 1) / 2) % 12; let part = 0; if (hh % 2) {
        if (ii > 40)
            part = 1;
    }
    else {
        if (ii < 20)
            ...
            part = 2;
    } return { index: [sc, part], char: this.cdz[sc] + des[part] }; }
    getSelfQi(tg, dz) { const b = [[2, 3, 5, 6, 5, 6, 8, 9, 11, 0], [3, null, 6, null, 7, null, 9, null, 0, null], [11, 11, 2, 2, 2, 2, 4, 4, 8, 8], [7, 7, 10, 10, 10, 4, 1, 1, 4, 4], [4, 4, 7, 7, null, null, 10, 10, 1, 1], [6, 6, 9, 9, 9, 9, 0, 0, 3, 3], [8, 8, 11, 11, 11, 11, 2, 2, 5, 5]]; for (let i = 0; i < 7; i++)
        if (b[i][tg] === dz)
    ...
    GetInfo(gd, yy, mm, dd, hh, mt = 0, ss = 0) {
        if (![0, 1].includes(gd))
            return {};
        const gz = this.GetGanZhi(yy, mm, dd, hh, mt, ss);
        if (gz.length === 0)
            return {};
        const [tg, dz, jd, jq, ix] = gz;
        const ret = { sex: gd };
        const xw = this.GetXiongWang(tg[2], dz[2]), pn = tg[0] % 2, big_tg = [], big_dz = [];
        ...
        ret.dz_cg = [];
        const gods = [], dzGod = [], dzMain = [], self = [], ny = [];
        for (let i = 0; i <= 3; i++) {
            ret.bazi.push([this.ctg[tg[i]], this.cdz[dz[i]]]);
            gods[i] = this.GetTenGod(tg[2], tg[i]);
            const cg = this.dzcg[dz[i]], g = [], chars = [];
            for (const x of cg) {
                g.push(this.GetTenGod(tg[2], x));
                chars.push(this.ctg[x]);
            }
            ret.dz_cg[i] = { index: cg, char: chars };
            dzMain.push(this.GetTenGod(tg[2], this.dztg[dz[i]]));
            dzGod[i] = g;
        ...
        ret.na_yin = ny;
        ret.xw = xw;
        ret.gong = this.GetGong(tg[0], dz[1], dz[3]);
        ret.shen_gong = this.GetShenGong(tg[0], dz[1], dz[3]);
        ret.tai_xi = this.GetTaiXi(tg[2], dz[2]);
        ret.birth_jd = jd;
        ret.jq_table = jq;
        ret.jq_ix = ix;
        ret.tg_cg_god = gods;
        ...
        ret.big_god = [];
        ret.big_cs = [];
        const zi = this.GetZodiac(mm, dd);
        ret.xz = zi === false ? '' : this.cxz[zi];
        ret.sx = this.csa[dz[0]];
        for (let i = 0; i < 12; i++) {
    ...
    } for (const dzEntry of info.dz_cg) {
        const arr = dzEntry.index, count = arr.length, blocks = count === 1 ? [18] : count === 2 ? [11, 7] : [9, 6, 3];
        arr.forEach((tg, i) => { const fen = tg % 2 === 1 ? 12 : 18; wx[this.GetTgWx(tg)] += fen * blocks[i] / 18; });
    } if (!noNaYin)
        for (const n of info.na_yin)
            wx[n[1]] += n[2] / 20; return wx; }
}
exports.LocalPaipan = LocalPaipan;
